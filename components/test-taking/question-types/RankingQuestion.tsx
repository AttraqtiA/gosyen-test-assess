  ...
  return (
    <ol className="grid gap-2">
      {current.map((id, index) => (
        <li key={id} className="flex items-center justify-between rounded-md border border-[var(--border)] bg-[var(--surface-soft)] p-3 text-sm">
          <span>
            {index + 1}. {labels.get(id) ?? id}
          </span>
          <span className="flex gap-2">
            <button type="button" className="text-[var(--muted)] hover:text-[var(--foreground)]" onClick={() => move(id, -1)}>
              Up
            </button>
            <button type="button" className="text-[var(--muted)] hover:text-[var(--foreground)]" onClick={() => move(id, 1)}>
              Down
